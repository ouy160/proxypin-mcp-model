...
ProxyPin MCP Server 默认监听 **9101** 端口，SSE 传输协议，无需额外安装任何依赖。
...
### 快速调试：直接发送 JSON-RPC 到 `/message`

无需 MCP 客户端，直接用 `curl` 发送原始 JSON-RPC 消息到 `/message` 端点即可快速测试：

```bash
# 1. 连接 SSE 获取 sessionId（另开终端或后台运行）
curl -N http://127.0.0.1:9101/sse
# → 输出: event: endpoint\ndata: http://127.0.0.1:9101/message?sessionId=xxxxx

# 2. 通过 POST /message?sessionId=xxxxx 调用任意工具
curl -X POST "http://127.0.0.1:9101/message?sessionId=xxxxx" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"get_request_list","params":{}}'

# 3. 无状态单行调用（省略 sessionId 即可，无需持久连接）
curl -X POST http://127.0.0.1:9101/message \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"list_breakpoints","params":{}}'
```

**提示**：`/message` 端点的 HTTP 响应体直接返回 JSON-RPC 结果，无需维持 SSE 长连接。`sessionId` 只在需要双向 SSE 事件（如断点拦截通知）时才必须传入；只读工具（查询、搜索、分析）省略 `sessionId` 即可。
...
启用请求重写规则或请求解密时，界面偶尔会出现闪烁或撕裂现象。这是因为大量状态更新与抓包引擎共享 UI 线程所致。不影响抓包数据或重写逻辑的正确性。

### 抓取接口卡顿

在高流量负载下，请求列表在新增请求时可能出现短暂卡顿。列表项到达数千条时尤为明显。建议定期使用 `clear_requests` 工具（或 MCP 的 `clear_requests`）清理列表。

---

## 抓取 Claude Desktop 流量

Claude Desktop（及其他 Electron 应用）使用 Node.js 自带证书存储。有两种方式拦截其 HTTPS 请求：

### 1. 证书信任（NODE_EXTRA_CA_CERTS）

1. 导出 ProxyPin CA 证书（应用菜单 → HTTPS → 导出根证书 → PEM 格式）
2. 在启动 Claude 前设置环境变量 `NODE_EXTRA_CA_CERTS`：
...
然后重启 Claude Desktop。Claude 的所有 HTTPS 请求都会被 ProxyPin 抓取。

### 2. 代理规则 + ProxyBridge

如需更灵活的控制（选择性路由、无需修改证书存储），可使用 代理规则 + ProxyBridge 组合：

- **代理规则** — 定义需要拦截的请求（按域名、进程或 URL 模式）
- **ProxyBridge** — 配套工具，将 Claude 流量桥接到 ProxyPin 的代理，无需修改 Node.js 证书

将 ProxyBridge 配置为通过 `127.0.0.1:9099`（ProxyPin 默认代理端口）转发流量，并设置代理规则目标为 `api.anthropic.com`（或 `*` 拦截全部）。详细配置见 ProxyBridge 文档。

> **注意**：Claude 的 MCP 客户端连接默认不经过系统代理——需要在 `claude_desktop_config.json` 中配置 MCP 服务器地址（参见上方[连接](#连接)章节）。
